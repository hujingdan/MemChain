...
class Emotion(Base):
    """
    情绪记录模型。
    每次发布记忆时记录一条情绪数据，用于绘制情绪时间轴折线图。
    """
    __tablename__ = "emotions"

    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id = Column(String(36), nullable=False, default="demo_user")  # Phase 2改为外键
    entry_id = Column(String(36), nullable=True)  # 关联的笔记ID（可选）
    score = Column(Integer, nullable=False)       # 情绪分值 -100~+100
    label = Column(String(20), nullable=False)    # 情绪标签key
    note = Column(String(200), nullable=True)     # 备注
    recorded_at = Column(DateTime, default=datetime.now)

    def __repr__(self):
        return f"<Emotion {self.label}({self.score}) at {self.recorded_at}>"
